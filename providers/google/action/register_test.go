...
import (
	...
	"github.com/go-modulus/auth/providers/google/action/mocks"
...
func TestRegister_Execute(t *testing.T) {
	t.Run(
		"successful registration with new user", func(t *testing.T) {
			...
			creatorMock.On(
				...
				mock.Anything,
				mock.AnythingOfType("action.User"),
			).Return(expectedUser, nil)

			pair, err := register.Execute(
	...
	t.Run(
		"registration with existing user", func(t *testing.T) {
			t.Skip("requires real Google OAuth - use for integration testing")
			...
			creatorMock.On(
				...
				mock.Anything,
				mock.AnythingOfType("action.User"),
			).Return(existingUser, action.ErrUserAlreadyExists)
	...
	t.Run(
		"user creation failure", func(t *testing.T) {
			t.Skip("requires real Google OAuth - use for integration testing")
			...
			creatorMock.On(
				...
				mock.Anything,
				mock.AnythingOfType("action.User"),
			).Return(action.User{}, creationError)

			_, err := register.Execute(
				ctx, action.RegisterInput{
	...
	t.Run(
		"unit test - user creator mock validation", func(t *testing.T) {
			ctx := context.Background()
