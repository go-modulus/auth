...
```
It should:
* implement the `github.com/go-modulus/auth/providers/google/action.UserCreator` interface.
* return `github.com/go-modulus/auth/providers/google/action.ErrUserAlreadyExists` error if the user with the same email already exists in the database. In this case it should return the existing user.
...
5. In the `internal/graphql/resolver/auth.resolvers.go` edit all generated resolvers adding a call to the appropriate resolvers from the `github.com/go-modulus/auth/providers/google/graphql.Resolver` struct. For example, change
```go
